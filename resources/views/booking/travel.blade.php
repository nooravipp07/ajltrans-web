...
<head>
    ...
    <title>Booking Travel - AJL Trans</title>
    @vite(['resources/css/app.css', 'resources/js/app.js'])
    <script defer src="https://unpkg.com/alpinejs@3.x.x/dist/cdn.min.js"></script>
</head>
<body class="bg-background antialiased font-sans" x-data="bookingForm('travel')" @vehicle-selected.window="handleVehicleSelected($event.detail)">
                ...
                <div class="lg:col-span-2">
                    ...
                    <div class="bg-white rounded-3xl shadow-xl shadow-slate-200/50 overflow-hidden">
                        <div class="bg-primary p-8 text-white">
                            <h1 class="text-3xl font-display font-bold mb-2">Form Travel</h1>
                            <p class="text-white/70">Layanan antar jemput door-to-door antar kota.</p>
                        </div>
                        ...
                        <form @submit.prevent="submitForm" class="p-8 space-y-8">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 border-t border-slate-100 pt-8" x-show="customerStatus !== 'blacklisted'">
                                ...
                                <div class="space-y-2">
                                    <label class="block text-sm font-bold text-slate-700 uppercase tracking-wider">Tanggal Perjalanan</label>
                                ...
                                <div class="space-y-2">
                                    <label class="block text-sm font-bold text-slate-700 uppercase tracking-wider">Jumlah Unit / Trip</label>
                                ...
                                <div class="space-y-2 md:col-span-2">
                                    <label class="block text-sm font-bold text-slate-700 uppercase tracking-wider">Alamat Penjemputan</label>
                                    <input type="text" x-model="formData.alamat_jemput" class="w-full rounded-xl border-slate-200 focus:border-primary focus:ring-primary py-3" placeholder="Masukkan alamat lengkap penjemputan..." required>
                                </div>
                                <div class="space-y-2 md:col-span-2">
                                    <label class="block text-sm font-bold text-slate-700 uppercase tracking-wider">Alamat Tujuan</label>
                                    <input type="text" x-model="formData.alamat_tujuan" class="w-full rounded-xl border-slate-200 focus:border-primary focus:ring-primary py-3" placeholder="Masukkan alamat lengkap tujuan..." required>
                                </div>
                                ...
                                <div class="md:col-span-2 p-6 bg-primary/5 rounded-2xl border border-primary/10" x-show="selectedVehicleData">
                                    ...
                                    <div class="flex justify-between items-center">
                                        <div>
                                            <p class="text-sm font-bold text-slate-500 uppercase tracking-widest">Estimasi Biaya Travel</p>
                                            <p class="text-xs text-slate-400" x-text="'Harga per Trip: Rp ' + formatNumber(getCurrentPrice())"></p>
                                        </div>
                                        <div class="text-right">
                                            <p class="text-3xl font-display font-extrabold text-primary" x-text="'Rp ' + formatNumber(calculateTotal())"></p>
                                            <p class="text-[10px] text-slate-400 italic">Penjemputan / Antar Bandara</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="pt-8">
                                <button type="submit" :disabled="customerStatus === 'blacklisted' || submitting || !isSelectedServiceAvailable()" class="w-full py-4 bg-primary text-white rounded-2xl font-bold text-lg hover:bg-primary-dark transition-all shadow-lg shadow-primary/25 disabled:opacity-50 disabled:cursor-not-allowed">
                                    <span x-show="!submitting">Pesan Travel</span>
        ...
        function bookingForm(category) {
            return {
                formData: {
                    ...
                    kategori: 'travel',
                    service_type: '{{ $defaultServiceType ?? 'travel_bandara' }}',
                    tanggal_mulai: '',
                    jam_mulai: '',
                    durasi: 12,
                    durasi_hari: 1,
